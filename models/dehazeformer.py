...
def vgg19_image_preproc(tensor_images):
    """
...
class DehazeFormer(nn.Module):
	def __init__(self, in_chans=3, out_chans=4, window_size=8,
		...
		# pass through pretrained vgg19 to get conv1_2 activation
		self.extractor_out_W = 224
		self.extractor_out_H = 224
		self.extractor_out_C = 64
		self.vgg_extractor = vgg19_conv1_2_feature_extractor()
		if torch.cuda.is_available():
			self.vgg_extractor.to("cuda")
		...
		self.patch_embed = PatchEmbed(
			patch_size=1, in_chans=in_chans+self.extractor_out_C, embed_dim=embed_dims[0], kernel_size=3)
	...
	def forward(self, x):
		...
		if self.upsampler is None:
			...
			Hs = H / self.extractor_out_H
			Ws = W / self.extractor_out_W
			self.upsampler = Upsample(size=(H, W), mode='bilinear')
		
		x_vgg_preproc = vgg19_image_preproc(x)
