...
parser.add_argument('--checkpoint', default=None, type=str, help='load the existing checkpoint')
...
if __name__ == '__main__':
	...
	if args.checkpoint:
		network.load_state_dict(torch.load(args.checkpoint)['state_dict'])
	...
	if not os.path.exists(os.path.join(save_dir, args.model+'.pth')):
		...
		for epoch in tqdm(range(setting['epochs'] + 1)):
			...
			if (epoch+1) % setting['eval_freq'] == 0:
				avg_psnr = valid(val_loader, network)
				print("avg_psnr", avg_psnr)
				
				writer.add_scalar('valid_psnr', avg_psnr, epoch+1)

				if avg_psnr > best_psnr:
					best_psnr = avg_psnr
					torch.save({'state_dict': network.state_dict()},
                			   os.path.join(save_dir, args.model+'_batch'+str(setting['batch_size'])+'_epoch'+str(setting['epochs'])+'_'+'_vgg19_best.pth'))
			...
			if (epoch+1) % 5 == 0:
				torch.save({'epoch': epoch+1,
							...
							'optimizer_state_dict': optimizer.state_dict()},
                			   os.path.join(save_dir, args.model+'_batch'+str(setting['batch_size'])+'_epoch'+str(setting['epochs'])+'_'+'_vgg19_temp.pth'))
